      ...
      <main className="flex-1 w-full px-4 sm:px-6 lg:px-10 xl:px-14 pb-16">
        ...
        >
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select
              ...
              className="h-8 rounded-lg border border-border bg-card px-2.5 text-sm text-foreground outline-none focus:ring-2 focus:ring-violet-500/30 max-w-48 truncate"
            >
        ...
        {filtered.length > 0 ? (
          <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4 space-y-4">
            {filtered.map(({ asset, eventTitle, company }, i) => (
              <div key={asset.id} className="break-inside-avoid">
                <div className="relative group">
                  ...
                  <div className="absolute top-2 left-2">
                    <span className={`text-xs rounded-full px-2 py-0.5 font-medium ${companyColor(company)}`}>
                      {company}
                    </span>
                  </div>
                </div>
